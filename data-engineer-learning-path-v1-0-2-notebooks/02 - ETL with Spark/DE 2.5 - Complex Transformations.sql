...
SELECT collect_set(event_name) AS event_history
FROM exploded_events;


-- COMMAND ----------

SELECT collect_set(items.item_id) AS cart_history
FROM exploded_events

-- COMMAND ----------

SELECT flatten(collect_set(items.item_id)) AS cart_history
FROM exploded_events

-- COMMAND ----------

SELECT array_distinct(flatten(collect_set(items.item_id))) AS cart_history
FROM exploded_events
...
SELECT array_distinct(flatten(collect_set(items.item_name))) AS cart_history2
FROM exploded_events;
...
SELECT user_id,
  collect_set(event_name) AS event_history,
  array_distinct(flatten(collect_set(items.item_name))) AS cart_history2
FROM exploded_events
GROUP BY user_id
...
-- MAGIC
-- MAGIC
-- MAGIC exploded_salesDF.where(col("order_id")==288311).show()
