...
SELECT count(*), count(DISTINCT user_id), count(DISTINCT user_first_touch_timestamp), count(DISTINCT email), count(DISTINCT updated)
FROM users_dirty

-- COMMAND ----------

SELECT count(DISTINCT *), count(DISTINCT user_id), count(DISTINCT user_first_touch_timestamp), count(DISTINCT email), count(DISTINCT updated)
FROM users_dirty
...
SELECT *
FROM users_dirty
WHERE (user_id, user_first_touch_timestamp, email, updated) IN
...
-- MAGIC %python
-- MAGIC display(usersDF.distinct().count())
...
SELECT count(*), count(DISTINCT user_id), count(user_id), count(user_id) - count(DISTINCT user_id) AS diffferents_in_duplicate
FROM users_dirty;

-- COMMAND ----------

SELECT count(*), count(DISTINCT user_first_touch_timestamp), count(user_first_touch_timestamp), count(user_first_touch_timestamp) - count(DISTINCT user_first_touch_timestamp) AS diffferents_in_duplicate
FROM users_dirty;

-- COMMAND ----------

SELECT count(*), count(DISTINCT email), count(email), count(email) - count(DISTINCT email) AS diffferents_in_duplicate
FROM users_dirty;

-- COMMAND ----------

SELECT count(*), count(DISTINCT updated), count(updated), count(updated) - count(DISTINCT updated) AS diffferents_in_duplicate
FROM users_dirty;

-- COMMAND ----------

SELECT max(DISTINCT user_id), max(DISTINCT user_first_touch_timestamp), email, updated
FROM users_dirty
WHERE (user_id, user_first_touch_timestamp, email, updated) IS NOT NULL
GROUP BY email, updated
...
SELECT email
  FROM deduped_users
  GROUP BY email
  HAVING COUNT(email) > 1 OR email IS NULL;
...
SELECT user_first_touch_timestamp, first_touch, email,
