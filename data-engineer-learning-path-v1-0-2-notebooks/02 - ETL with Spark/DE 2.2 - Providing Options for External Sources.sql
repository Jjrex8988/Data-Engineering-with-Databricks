...
SELECT order_id, email, transactions_timestamp, total_item_quantity, purchase_revenue_in_usd, unique_items, items
FROM sales_csv
GROUP BY order_id, email, transactions_timestamp, total_item_quantity, purchase_revenue_in_usd, unique_items, items

-- COMMAND ----------

SELECT count(order_id), count(email), count(transactions_timestamp), count(total_item_quantity), count(purchase_revenue_in_usd), count(unique_items), count(items)
FROM sales_csv

-- COMMAND ----------

SELECT count(DISTINCT order_id), count(DISTINCT email), count(DISTINCT transactions_timestamp), count(DISTINCT total_item_quantity), count(DISTINCT purchase_revenue_in_usd), count(DISTINCT unique_items), count(DISTINCT items)
FROM sales_csv
...
SELECT order_id, email, transactions_timestamp, total_item_quantity, purchase_revenue_in_usd, unique_items, items, COUNT(*) as duplicate_count
FROM sales_csv
GROUP BY order_id, email, transactions_timestamp, total_item_quantity, purchase_revenue_in_usd, unique_items, items
HAVING COUNT(*) > 1;
...
SELECT count(distinct purchase_revenue_in_usd)
FROM sales_csv


-- COMMAND ----------

SELECT count(distinct unique_items)
FROM sales_csv
...
SELECT count(distinct total_item_quantity)
FROM sales_csv
