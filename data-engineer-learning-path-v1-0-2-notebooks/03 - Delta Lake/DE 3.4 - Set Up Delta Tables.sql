...
-- MAGIC %python
-- MAGIC display(dbutils.fs.ls(f"{DA.paths.user_db}"))

-- COMMAND ----------

-- MAGIC %python
-- MAGIC display(dbutils.fs.ls(f"{DA.paths.datasets}"))

-- COMMAND ----------

-- MAGIC %python
-- MAGIC display(dbutils.fs.ls(f"{DA.paths.datasets}/ecommerce"))
...
CREATE OR REPLACE TABLE purchase_dates8988 (
  id STRING, 
  transaction_timestamp STRING, 
  price STRING,
  date DATE GENERATED ALWAYS AS (
...
INSERT INTO purchase_dates8988
SELECT order_id AS id, transaction_timestamp, purchase_revenue_in_usd AS price, cast(cast(transaction_timestamp/1e6 AS TIMESTAMP) AS DATE)
FROM sales;
...
CREATE OR REPLACE TABLE users_historical8988 AS
SELECT * FROM PARQUET.`${da.paths.datasets}/ecommerce/raw/users-historical`;

SELECT * FROM users_historical8988;
